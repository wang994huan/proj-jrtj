	...
	require(["jquery", "sw"], function($, Swiper) {
		...
		$(document).scroll(function() {
			if ($(document).scrollTop() > 406) {
				...
				$(".hori-cate-area").css("display", "block");
			} else {
				$(".tab-area").removeClass("fixed");
				$(".hori-cate-area").css("display", "none");
			}
		})
		let p = 1;
		let sort = "new";
			...
			$.ajax({
				...
				success: function(d) {
					...
					list1.forEach(function(item, i) {
						...
						$(item).parent().attr("href", "deal/index.html?goodId=" + d.data[i].goods_id + "&title=" + d.data[i].title +
							"&href=" + d.data[i].click_url +
							"&src=" +
							d.data[i].img_url + "&desc=" + d.data[i].seo_desc + "&yishou=" + d.data[i].volume + "&yuanjia=" + d.data[
								i].org_price + "&price=" +
							d.data[i].price);
						$(item).children(".title").text(d.data[i].rec_desc);
		...
		if(username){
			$(".login:first").css("display","none");
			$(".login:last").css("display","none");
		}
	})
})
