	...
	require(["jquery", "sw"], function($, Swiper) {
			...
			$.ajax({
				...
				success: function(d) {
					...
					list1.forEach(function(item, i) {
						...
						$(item).parent().attr("href", "../deal/index.html?goodId=" + d.data[i].goods_id + "&title=" + d.data[
								i].title +
				...
				$.ajax({
					...
					success: function(d) {
						...
						for (let i = 0; i < d.data.length; i++) {
							...
							html += "<a class='fl goods-item' href='../deal/index.html?goodId=" + d.data[i].goods_id + "&title=" + d
								.data[
									i].title +
								"&href=" + d.data[i].click_url +
								"&src=" +
		...
		$(".search-btn").on("click", function() {
			...
			$.ajax({
				...
				data: searchData,
				success: function(d) {
					window.location = "search/index.html?k=" + searchData.k + "&sort=" + searchData.sort + "&p=" + searchData
						.p;
				}
			})
		});
